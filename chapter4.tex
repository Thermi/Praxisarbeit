...
The resource is defined using the class, the device that should be mounted, the mountpoint
and the filesystem type. Additionally I told pacemaker to monitor the resource in 20 seconds intervals.
\begin{lstlisting}
...
\end{lstlisting}
''pcs resource show cluster-data'' shows more information about the resource:
\begin{lstlisting}
 Resource: cluster-data (class=ocf provider=heartbeat type=Filesystem)
  Attributes: device=/dev/vdb1 directory=/var/www/mirror/ fstype=xfs 
  Operations: start interval=0s timeout=60 (cluster-data-start-timeout-60)
              stop interval=0s timeout=60 (cluster-data-stop-timeout-60)
              monitor interval=20 (cluster-data-monitor-interval-20)
\end{lstlisting}
\paragraph{lighttpd}

\paragraph{stonith}
\paragraph{ordering the resources}
\paragrap{cluster test}
% drbd
% file systems
% lighttpd
% stonith
