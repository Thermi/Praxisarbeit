...
\paragraph{DRBD}
DRBD stands for Dynamic Redundant Block Devices and is an open source technology for developed by LINBIT.
It provides data replication over a cluster of two hosts.  It provides a transparent layer over a given block device. When the transparent layer is written to, the write is both executed on the local block device and on a remote, clustered device. As of the time of the writing, DRBD only works good for two nodes, but scales bad beyond it. In the next version of DRBD, support vor arbitrary numbers of cluster members will be added, which enables cheap distributed storage.
