...
to fencing in a virtualised environment and is the successor to . The upstream of the software is at Github
\footnote{\url{https://github.com/ClusterLabs/fence-virt}}. fence-virt already supports
fencing over multicast IP addresses and serial connections.
Fencing over multicast IP addresses is needed in an environment, where the cluster members might
be distributed over different \ac{VM} hosts. Addressing the fencing %server or agent?
over multicast enables fencing of cluster members, of which it is not known where
they run. Using multicast makes sure that the member gets fenced, independent of the
